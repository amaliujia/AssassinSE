...
public class QryEval {
        ...
        if(params.containsKey("fbInitialRankingFile")){
           ...
           String outputRankingFilePath = params.get("fbExpansionQueryFile");
            ...
            for (int i = 0; i < keys.size(); i++) {
                ...
                String que = queries.get(i);
                qTree = parseQuery(que, model);
                ResultsAfterFeedback(key, qTree.evaluate(model), qTree, docNum, termNum);
            }
  ...
  static void printResults(String queryID, QryResult result, Qryop qTree) throws IOException {
     ...
     * @throws IOException
     */
    static void ResultsAfterFeedback(String queryID, QryResult result, Qryop qTree, int docNum, int termNum) throws IOException {
     ...
     */
        try {
            if (result.docScores.scores.size() < 1) {
                ...
                SDPesudoFeedBackEngine feedBackEngine = new SDPesudoFeedBackEngine(docNum, termNum, folder, queryID);
